...
def SudokuDrawer(grid: Array[Array[String]], frame: JFrame, panel: JPanel): Array[Array[String]] = {
  ...
  solveButton.addActionListener((e: ActionEvent) => {
      var s = "["
      grid.zipWithIndex.foreach{ case(row, i) =>
        row.zipWithIndex.foreach{ case(col, j) =>
          s += (if(j == 0) "[" else ",")
          if (grid(8 - i)(j).substring(1) == " ") s = s + "_"
          else s = s + grid(8 - i)(j).substring(1)
          if(j == 8)s += (if(i != 8)"]," else "]].")
        }
      }
      val pw = new PrintWriter(new File("/Users/mohamedehab/Desktop/file"))
      pw.write(s)
      pw.close()
      val file = new File("/Users/mohamedehab/Desktop/file")
      ...
      grid.zipWithIndex.foreach { case (row, i) =>
        row.zipWithIndex.foreach { case (col, j) =>
          k = k + 1
          grid(8-i).update(j, "b".concat(s2(k).toString))
          k = k + 1
          if (j == 8) k = k + 2
        }
      }
...
def SudokuController(move: String, grid: Array[Array[String]], player: Int): Any = {
  ...
  SudokuIsValid(move, grid, """([1-9])([a-i]) ([1-9])""".r, """([1-9])([a-i])""".r) match {
    ...
    case true => SudokuUpdateGrid(move.charAt(0).asDigit - 1, map(move.charAt(1)), grid
      , if(move.length == 4) move.substring(3) else " ")
  }
}